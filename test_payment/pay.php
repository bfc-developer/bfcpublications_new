...
// echo "<pre>";
// print_r($resData); 
$response = file_get_contents($resData);

print_r($response); die;
if (json_last_error() === JSON_ERROR_NONE) {
  // Extract payments URL
  $paymentsUrl = $resData->payments->url;
  echo "Payments Response: " . $paymentsUrl . "\n";
  // Initialize cURL for payments request
  $curl = curl_init();
  curl_setopt_array($curl, [
      CURLOPT_URL => $paymentsUrl,
      CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => '',
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 0,
  ...
  // Decode payments response
  $paymentsData = json_decode($paymentsResponse, true);
  if (json_last_error() === JSON_ERROR_NONE) {
      // Print or process the payments data
   //   print_r($paymentsData);

      // Example: Extract and display transaction details
      if (isset($paymentsData['transactions']) && is_array($paymentsData['transactions'])) {
          foreach ($paymentsData['transactions'] as $transaction) {
...
if(isset($resData->cf_order_id) && $resData->cf_order_id !=''){
  ...
  // echo   $paymentsUrl = $resData->payments->url;
    $curl = curl_init();
    curl_setopt_array($curl, [
        CURLOPT_URL => $paymentsUrl,
        CURLOPT_RETURNTRANSFER => true,
